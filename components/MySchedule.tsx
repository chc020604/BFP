...
import { MOCK_EVENTS } from '../constants'; // Use mock data to simulate saved events

const MySchedule: React.FC = () => {
  // Simulate a subset of events as "Saved"
  const savedEvents = MOCK_EVENTS.slice(0, 3);
      ...
      <div className="relative">
        ...
        <div className="space-y-12">
            {savedEvents.map((event, index) => (
                <div key={event.id} className="relative flex items-start pl-12 md:pl-20 group">
                    ...
                    <div className="hidden md:block absolute left-0 top-6 -translate-x-[120%] text-right w-24">
                        ...
                        <p className="text-xs text-slate-400">{index === 0 ? 'D-5' : 'Upcoming'}</p>
                    </div>
            ...
            <div className="pl-12 md:pl-20">
                <button className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 hover:border-rose-300 hover:text-rose-500 transition-all flex items-center justify-center gap-2">
